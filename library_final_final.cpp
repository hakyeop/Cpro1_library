...
typedef struct // 책 구조체
{
	...
	int lent_num; // 대출한 사람을 적어두는 변수 (대출한 사람이 없는경우 0)
...
void return_book(book* books, int book_count, int login_num);
//-----------------------------------------------------------------
...
int main(void)
{
	...
	while (1)
	{
		...
		case 5:
			...
			return_book(books, book_count, login_num);
...
void checkout_book(book* books, int book_count, int login_num)
{
	...
	printf("대출 가능한 도서 목록:\n");
	for (int i = 0; i < book_count; i++) {
		if (books[i].lent_num == 0) {
			printf("도서 ID: %d, 도서 이름: %s\n", books[i].bookid, books[i].bookname);
		}
	}
	...
	if (found_index != -1) {
		// 도서를 찾은 경우 대출 처리
		if (books[found_index].lent_num == 0) {
			books[found_index].lent_num = login_num;
...
void return_book(book* books, int book_count, int login_num)
{
	...
	printf("반납 가능한 도서 목록:\n");
	for (int i = 0; i < book_count; i++) {
		if (books[i].lent_num == login_num ) {
			printf("ID: %d, 도서 이름: %s, 대출자 ID: %d\n", books[i].bookid, books[i].bookname, books[i].lent_num);
		}
	}
	...
	if (found_index != -1) {
		// 도서를 찾은 경우 반납 처리
		if (books[found_index].lent_num == login_num) {
			books[found_index].lent_num = 0;
			printf("# 도서가 성공적으로 반납되었습니다.\n");
		}
		else {
			printf("@ 대출한 아이디와 불일치합니다!!\n");
		}
	}
	else {
		printf("@ 해당 도서를 찾을 수 없습니다.\n");
